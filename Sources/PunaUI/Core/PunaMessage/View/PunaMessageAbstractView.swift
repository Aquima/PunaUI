...
class PunaMessageAbstractView: UIView, PunaMessageView, UITextViewDelegate, PunaBulletViewDelegate {
    weak var delegate: PunaMessageViewDelegate?
    ...
    private(set) var config: PunaMessageViewConfig
    init(withConfig config: PunaMessageViewConfig) {
        self.config = config
    ...
    func updateView() {
        ...
        self.bodyTextView.setPunaStyle(style: config.bodyStyle)
        ...
        if let icon = config.icon {
            PunaIconsProvider.loadIcon(name: icon, placeItInto: self.iconView)
        }
        ...
        if config.titleText == nil || config.titleText!.isEmpty {
            ...
            self.titleLabel.setPunaStyle(style: config.titleStyle)
            self.titleLabel.text = config.titleText
    ...
    func mapBulletToView(for bullet: PunaBullet, at index: Int) -> PunaBulletView {
        let bulletView = PunaBulletView(frame: .zero)
    ...
    func getBodyText(style: PunaFontStyle) -> NSAttributedString {
        let body = config.bodyText ?? ""
        return getBodyAttributedText(text: body, bodyLinks: config.bodyLinks)
    }

    func getBodyAttributedText(text: String, bodyLinks: PunaBodyLinks?) -> NSAttributedString {
