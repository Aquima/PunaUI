...
class PunaTagSimpleView: PunaTagAbstractView {
    ...
    override func loadNib() {
        ...
        bundle.loadNibNamed("PunaTagSimpleView", owner: self, options: nil)
    }
    ...
    func setupRightContent() {
        if let rightButtonImageName = config.rightButtonImageName {
            PunaIconsProvider.loadIcon(name: rightButtonImageName) { image in
    ...
    func updateLeftContentView() {
        if let leftContent = config.leftContent {
            ...
            self.leftContentView = PunaTagLeftContentFactory.generateLeftContentView(for: leftContent)
