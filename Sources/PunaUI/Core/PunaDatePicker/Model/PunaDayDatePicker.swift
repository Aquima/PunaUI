...
protocol PunaDayDatePickerDelegate: class {
    func didSelectEnabledDay(_ day: Date?)
}

@objc class PunaDayDatePicker: NSObject {
    ...
    weak var delegate: PunaDayDatePickerDelegate?
    ...
    func getDaysInMonth(_ currentDate: Date, _ selectedDate: Date?) -> [PunaDayDatePicker] {
        guard let monthData = try? PunaMonthDatePicker.getMonthData(currentDate) else {
        ...
        var daysToRenderInMonth: [PunaDayDatePicker] = (1..<(daysNumberInMonth + offsetToInitialRow)).map { day in
            ...
            return PunaDayDatePicker(date: date, number: dateFormatter.string(from: date), selected: selected, isCurrentMonth: isCurrentMonth, endDate: dueDate, isValid: isValid)
        }
    ...
    func generateStartOfNextMonth(using firstDayOfDisplayedMonth: Date, selectedDate: Date) -> [PunaDayDatePicker] {
        ...
        let days: [PunaDayDatePicker] = (1...additionalDays)
            .map {
                let date = calendar.date(byAdding: .day, value: $0, to: lastDayInMonth) ?? lastDayInMonth
                isValid = hasRange() ? dateIsInRange(date) : true

                return PunaDayDatePicker(date: date,
