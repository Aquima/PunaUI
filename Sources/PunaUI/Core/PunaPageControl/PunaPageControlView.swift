...
public protocol PunaScrollingPageControlDelegate: class {
    func viewForDot(at index: Int) -> UIView?
}

open class PunaPageControlView: UIView {
    ...
    open weak var delegate: PunaScrollingPageControlDelegate? {
        didSet {
            createViews()
        }
    }
    ...
    open var dotColor = UIColor.Puna.graySolid250 {
        didSet {
            updateColors()
        }
    }
    open var selectedColor = PunaStyleSheetManager.styleSheet.accentColor { didSet { updateColors() } }
    ...
    private func createViews() {
        ...
        dotViews = (0..<pages).map { index in
            delegate?.viewForDot(at: index) ?? PunaPageDotView(frame: CGRect(origin: .zero, size: CGSize(width: dotSize, height: dotSize)))
        }
    }
