...
class PunaCheckboxDefaultView: UIView, PunaCheckboxView {
    ...
    weak var delegate: PunaCheckboxViewDelegate?

    var config: PunaCheckboxViewConfig

    init(withConfig config: PunaCheckboxViewConfig, delegate: PunaCheckboxViewDelegate) {
        self.config = config
    ...
    func loadNib() {
       ...
       bundle.loadNibNamed("PunaCheckboxDefaultView", owner: self, options: nil)
    }

    override init(frame: CGRect) {
        self.config = PunaCheckboxViewConfig()
    ...
    func initialize() {
        self.label.setPunaStyle(style: PunaStyleSheetManager.styleSheet.bodyM(color: config.textColor))
        ...
        if let currentIcon = config.icon, !currentIcon.isEmpty {
            if let currentIconColor = config.iconColor {
                if self.leftBox.isHidden == false {
                    PunaIconsProvider.loadIcon(name: currentIcon) { checkboxIcon in
